...
namespace ConsoleApp1
{
    internal class Game
    {
        ...
        {
            move = 1;
            for (int cell = 1; cell <= maxMove; cell++)
            {
                board[cell] = cell.ToString();
            }
        }
        ...
        {
            int cell;
            string[] player = { "Игрок #1", "Игрок #2" };
            string[] playerMarker = { "X", "O" };

            Console.Clear();
            Console.ForegroundColor = ConsoleColor.Blue;
            Console.WriteLine(player[0] + ": " + playerMarker[0]);
            Console.ForegroundColor = ConsoleColor.Red;
            Console.WriteLine(player[1] + ": " + playerMarker[1]);
            Console.ResetColor();
            Console.WriteLine("");
            DrawBoard();
            ...
            if (board[cell] != playerMarker[0] && board[cell] != playerMarker[1])
            {
                board[cell] = playerMarker[move % 2];
                move++;
            }
            else
            {
        ...
        private static void WaitingMsg()
        {
            for (int i = 3; i > 0; i--)
            {
                Console.ForegroundColor = ConsoleColor.Yellow;
